...
raise notice 'inside update_piece_fts, target_id: %', $1;
   ...
   SET fts = to_tsvector('english', 
                          ...
                          COALESCE((SELECT string_agg(unaccent(regexp_replace(m.name, '/', ' ', 'g')), ' ') 
                                    FROM movements m 
                                    WHERE m.piece_id = $1), '') || ' ' ||
...
BEGIN
 ...
 formatted_query := replace(unaccent(regexp_replace(query, '/', ' ', 'g')), ' ', ' & ');  -- Apply unaccent and handle slashes before replacing spaces
 RETURN QUERY
